...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/AWS-Cloud%20Deployment-FF9900?logo=amazonaws&style=for-the-badge" />
  ...
  <img src="https://img.shields.io/badge/Payments-Mercado%20Pago-009EE3?style=for-the-badge&logo=mercadopago" />
...
Actualmente el proyecto se encuentra desplegado sobre infraestructura cloud real utilizando **AWS ECS Fargate + CloudFront + Route53 + Redis + Secrets Manager + MongoDB Atlas**, incluyendo pipelines de seguridad automatizados y arquitectura lista para producción.
...
The project is currently deployed on real cloud infrastructure using **AWS ECS Fargate + CloudFront + Route53 + Redis + Secrets Manager + MongoDB Atlas**, including automated security pipelines and production-ready architecture.
...
- despliegue cloud production-ready
- simulación de arquitectura enterprise moderna
...
- modern enterprise architecture simulation
- real DevSecOps implementation
...
| **Cloud** | AWS ECS Fargate, CloudFront, Route53, ECR, ElastiCache Redis, Secrets Manager, CloudWatch |
| **Infrastructure** | MongoDB Atlas, NAT Gateway, Elastic IP, HTTPS, custom domains |
...
Producción	MongoDB Atlas + Redis	ECS + Secrets Manager	Docker + AWS ECS
☁️ AWS Cloud Infrastructure

🇪🇸

El proyecto fue desplegado utilizando infraestructura cloud real en AWS con arquitectura moderna y endurecimiento de seguridad.
...
NAT Gateway
Elastic IP
MongoDB Atlas endurecido para producción

Características implementadas:

despliegue containerizado
backend escalable
frontend distribuido globalmente por CDN
logs centralizados
secretos protegidos fuera del código
Redis listo para rate limiting y token store
MongoDB Atlas protegido mediante IP fija AWS
HTTPS real con dominios personalizados

🇬🇧

The project was deployed using real AWS cloud infrastructure with modern architecture and hardened security.
...
NAT Gateway
Elastic IP
Production-hardened MongoDB Atlas

Implemented features:

containerized deployment
scalable backend
globally distributed frontend through CDN
centralized logs
secrets protected outside source code
Redis ready for rate limiting and token store
MongoDB Atlas protected through fixed AWS IP
real HTTPS with custom domains
...
La aplicación incluye integración profesional con Mercado Pago:
...
tickets automáticos
fallback mediante transferencia/efectivo

Arquitectura preparada para entorno productivo real.

🇬🇧

The application includes professional Mercado Pago integration:
...
bank transfer/cash fallback

Architecture prepared for real production environments.
...
Middleware	Helmet, HPP, mongoSanitize	XSS, NoSQLi, pollution	Node hardening
...
🔐 Autenticación adaptativa / Adaptive Authentication
...
Redis
ECS Fargate
arquitectura preparada para despliegues cloud enterprise
...
Redis
ECS Fargate
architecture ready for enterprise cloud deployments
...
Fullstack Developer — DevSecOps | AWS Cloud | Docker | MongoDB | React | Node.js | CI/CD | Cybersecurity
