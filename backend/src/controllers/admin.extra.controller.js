// src/controllers/admin.extra.controller.js
...
export const exportProducts = async (req, res, next) => {
  try {
    ...
    const { filename, contentType, stream } = await AdminExtraService.exportProducts(format);
...
export const exportUsers = async (req, res, next) => {
  try {
    const format = pickFormat(req);
    const { filename, contentType, stream } = await AdminExtraService.exportUsers(format);
...
export const exportSales = async (req, res, next) => {
  try {
    const format = pickFormat(req);
    const { filename, contentType, stream } = await AdminExtraService.exportSales(format, req.query);
    res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);
    res.setHeader("Content-Type", contentType);
    return stream.pipe(res);
  } catch (err) {
...
export const bulkUpdatePrices = async (req, res, next) => {
  try {
    ...
    req.logger?.info?.(`💹 Bulk price update executed: ${JSON.stringify(result)}`);
...
export const deleteInactiveUsers = async (req, res, next) => {
  try {
    ...
    const result = await AdminExtraService.deleteInactiveUsers({ days, dryRun });
...
export const importProductsCsv = async (req, res, next) => {
  try {
    if (!req.file?.path) {
      return res.status(400).json({ error: "Subí un archivo CSV con el campo 'file'." });
    }
